...
function RecruiterDashboard() {
  ...
  const fetchData = async () => {
    setLoading(true)
    try {
      const promises = [fetchAnalytics()]
      if (tabValue === 0) {
        promises.push(fetchTests())
      } else if (tabValue === 1) {
        promises.push(fetchQuestions())
      } else if (tabValue === 2 || tabValue === 3) {
        promises.push(fetchCompletedSessions())
      }
      ...
      <Container maxWidth="xl" sx={{ mt: 4, mb: 4 }}>
        ...
        <Grid container spacing={3} sx={{ mb: 4 }}>
          {[
            ...
            { label: 'Total Questions', value: analytics.totalQuestions, color: '#8b5cf6', icon: <Quiz /> },
            ...
            <Grid item xs={12} sm={6} md={2.4} key={index}>
              <Card sx={{
                ...
                <CardContent sx={{ p: 2 }}>
                  ...
                  <Typography color="textSecondary" variant="caption" sx={{ fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                    {stat.label}
                  </Typography>
                </CardContent>
              </Card>
                ...
                <Box
                  ...
                  onSubmit={async (e) => {
                    ...
                    try {
                      ...
                      const response = await api.put('/recruiter/me', payload)
