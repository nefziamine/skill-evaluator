...
function TestResults() {
  ...
  const calculateAverage = () => {
    const completed = sessions.filter((s) => s.isCompleted && s.score != null && s.totalPoints > 0)
    if (completed.length === 0) return 0
    const totalPercentage = completed.reduce((acc, s) => acc + (s.score * 100) / s.totalPoints, 0)
    return (totalPercentage / completed.length).toFixed(1)
  }
